...
function CreatePost() {
    ...
    return ( 
        <div className="bg-gray-800 p-2 sm:p-5 my-auto outline-none rounded-2xl shadow-inner shadow-gray-900 w-full min-[680px]:w-[50%] sm:mx-auto h-full overflow-y-scroll no-scrollbar">
            <h1 className="text-center sm:text-xl text-gray-400 bg-clip-text sm:my-4">Let's create new post!</h1>
            <form method="post" className="sm:w-[80%] sm:mx-auto flex flex-col mt-4 sm:mt-8" onSubmit={(e)=>handleSubmit(e)}>
                ...
                <div className="flex flex-col sm:flex-row sm:justify-center align-middle">
                ...
                <div className="flex flex-col sm:flex-row justify-center align-middle">
                    ...
                    <textarea rows={5} name="body" id="body" className="sm:w-[70%] bg-gray-700 text-gray-300 p-2 mb-8 outline-none rounded-2xl shadow-sm shadow-gray-900" placeholder="Enter your post description" value={formData.body} onChange={handleChange}/>
                    {
                        isSubmit ?
                ...
                <div className="w-full text-center">
                    <input type="submit" name="submit" id="submit" className="w-20 sm:w-24 ml-8 inline-block rounded-full p-1 font-bold font-serif bg-gray-700 text-blue-500 cursor-pointer hover:shadow-inner hover:shadow-gray-800 active:animate-bounce" value="Save"/>
                    <button type="reset" name="cancel" id="cancel" className="w-20 sm:w-24 ml-4 inline-block rounded-full p-1 font-bold font-serif  mb-1 bg-gray-700 text-blue-500 hover:shadow-inner hover:shadow-gray-800 active:animate-bounce" onClick={()=>{
                        setFormData(initialValue);
                        setIsSubmit(false);
                    }}>Cancel</button>
                </div>
...
function createPost(formData){
    ...
    axios.post('http://localhost:3030/posts',{
        ...
        alert(`Create new post - ${res.data.title}`);
    });
}